...
Output: 
Return Value: ""
Tip (In C) Don't worry about free, it will be done in our main().
...
Output: 
Return Value: [3]
Tips Google while YOURCODINGLANGUAGE Google for YOURCODINGLANGUAGE Google array.length YOURCODINGLANGUAGE (In C) Yes, you have to allocate for the struct AND then allocate for the array inside. :-)

(In C) Don't forget to set the size. ;-)
...
Output: 
Return Value: "{x: 2, y: 1, direction: 'down'}"
Tip (In C) In C, you can use snprintf.