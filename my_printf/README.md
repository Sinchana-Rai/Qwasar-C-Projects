...
NAME : int my_printf(char * restrict format, ...);
...
## Task
The my_printf function produces output according to different input formats.

## Description
User inputs dats of different types like String(s), Integer(i), Character(c), unsigned octal (o), unsigned decimal (u), unsigned hexadecimal (x) and the program reads the input and displays the data as the output.

## Installation
Compile the program with the command -> gcc -o my_printf my_printf.c

## Usage
To ececute the project:
 Compile the program with the command -> gcc -o my_printf my_printf.c
Then Execute the program with the command -> ./my_printf  
```
