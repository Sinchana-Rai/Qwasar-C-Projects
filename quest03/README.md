...
Objective: More pointers, more loops, more strings, and the beginning of struct. :-)
...
Tip Return value is connected to ASCII values ;-)
...
Output: 
Return Value: true
Tips (In C) In C, we have defined boolean on char so you can use the type boolean :) (In C) Curious about the integer_array type?

typedef struct s_integer_array {
  int size;
  int* array;
} integer_array;
integer_array_variable->size will give you the size of the array

integer_array_variable->array will give you the access to the array

integer_array_variable->array[0] => is the first cell of the array

Please do not define the struct in your code when sending to gandalf.

Google: what is a Boolean Google: sort ascending
